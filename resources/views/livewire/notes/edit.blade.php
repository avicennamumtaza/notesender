...
<div class="py-12">
    <div class="max-w-2xl mx-auto space-y-4 sm:px-6 lg:px-8">
        <form wire:submit='saveNote' class="space-y-4">
            ...
            <x-wui-textarea wire:model="noteBody" label="Your Note"
                placeholder="Share all your thoughts with your friend." />
            <x-wui-input icon="user" wire:model="noteRecipient" label="Recipient" placeholder="yourfriend@email.com"
                type="email" />
            <x-wui-input icon="calendar" wire:model="noteSendDate" type="date" label="Send Date" />
            <x-wui-checkbox label="Note Published" wire:model='noteIsPublished' />
            <div class="flex justify-between pt-4">
                <x-wui-button type="submit" secondary spinner="saveNote">Save Note</x-wui-button>
                <x-wui-button href="{{ route('notes.index') }}" flat negative>Back to Notes</x-wui-button>
            </div>
